...
export async function filterInventoryd(
  ...
  for (const [, value] of Object.entries(filtersData)) {
    ...
    if (command == '2') {
      totalTwo += 1;
      const tempInventory = combinedInventory.filter(function (item: any) {
  ...
  for (const [, value] of Object.entries(filtersData)) {
    ...
    // First filter
    if (command == '1') {
      combinedInventory = combinedInventory.filter(function (item: any) {
...
export async function sortDataFunction(
  ...
  function sortRunAlt(valueOne: any, ValueTwo: any) {
    if (isNaN(valueOne)) {
      valueOne = -90000000000
    }
    if (isNaN(ValueTwo)) {
  ...
  if (sortValue == 'Storages') {
    inventory.sort(function (a: any, b: any) {
